...
func (s *echoServer) AuthFuncOverride(ctx context.Context, fullMethodName string) (context.Context, error) {
	...
	if err != nil {
		if s.jwtSecret == "" ||
			fullMethodName == "/grpc.gomeetexamples.echo.Echo/Version" ||
			fullMethodName == "/grpc.gomeetexamples.echo.Echo/ServicesStatus" {
			return ctx, nil
		}
...
func (s *echoServer) AclFuncOverride(ctx context.Context, fullMethodName string, msg proto.Message) error {
	lFields := log.Fields{"fullMethodName": fullMethodName}
